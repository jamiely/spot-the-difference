...
import type { SpriteManager } from '../js/components/SpriteManager.js';
...
interface MockGame extends SpotTheDifferenceGame {
    ...
    modal: any;
    clearAllPreviousLevelData: ReturnType<typeof vi.fn>;
    clearDifferenceMarkers: ReturnType<typeof vi.fn>;
    clearOrphanedSprites: ReturnType<typeof vi.fn>;
    clearOrphanedMarkers: ReturnType<typeof vi.fn>;
    handleGameComplete: ReturnType<typeof vi.fn>;
    loadTemplateForSpotTheDifference: ReturnType<typeof vi.fn>;
    isTestMode: boolean;
    levelManager: any;
    currentTemplate: any;
}
...
describe('Sprite Cleanup Between Levels', () => {
    let game: MockGame;
    let mockGameBoard: MockElement;
    let mockLeftBoard: MockElement;
    let mockRightBoard: MockElement;

    beforeEach(() => {
        // Mock DOM elements
        const createMockElement = (id: string, className: string = ''): MockElement => ({
            id,
            className,
            style: { display: 'block' },
            appendChild: vi.fn(),
        ...
        game = new SpotTheDifferenceGame() as MockGame;
    });
    ...
    describe('clearOrphanedSprites', () => {
        it('should remove all game-sprite elements from both boards', () => {
            ...
            mockLeftBoard.querySelectorAll = vi.fn((selector: string) => {
                if (selector === '.game-sprite') return [leftSprite1, leftSprite2];
                return [];
            });

            mockRightBoard.querySelectorAll = vi.fn((selector: string) => {
    ...
    describe('loadNextLevel integration', () => {
        ...
        it('should clear data even in test mode', async () => {
            ...
            game.loadTemplateForSpotTheDifference = vi.fn().mockResolvedValue(undefined);

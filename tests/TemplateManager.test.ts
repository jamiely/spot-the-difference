...
import type { BackgroundDimensions, RenderDimensions } from '../js/utils/TemplateManager.js';
...
interface MockTemplateData extends TemplateData {
...
describe('TemplateManager', () => {
  ...
  beforeEach(() => {
    ...
    global.console = { warn: vi.fn(), log: vi.fn() } as any;
  ...
  it('should validate a valid template', () => {
    const validTemplate: TemplateData = {
      id: 'valid-template',
      name: 'Valid Template',
      background: 'bg.png',
      sprites: [{ id: 's1', src: 's1.png', x: 1, y: 2 }],
    };
  ...
  it('should invalidate a template with invalid sprite src', () => {
    const invalidTemplate: TemplateData = {
      id: 'test-template',
      name: 'Test',
      background: 'bg.png',
      sprites: [{ id: 's1', src: null as any, x: 1, y: 2 }],
    };
    expect(templateManager.validateTemplate(invalidTemplate)).toBe(false);
  });

  it('should get loaded templates', async () => {
    (templateManager as any).loadedTemplates = [{ id: 't1' }, { id: 't2' }];
  ...
  it('should get template by name', async () => {
    (templateManager as any).loadedTemplates = [{ name: 'Template One' }, { name: 'Template Two' }];
  ...
  it('should create template from current state', () => {
    const sprites: SpriteData[] = [{ id: 's1', src: 's1.png', x: 10, y: 20, width: 100, height: 100 }];
  ...
  it('should validate template with new background dimensions structure', () => {
    const validTemplate: MockTemplateData = {
      id: 'test-template',
      name: 'Test Template',
      background: 'test.png',
      backgroundDimensions: {
      ...
      sprites: [{ id: 'sprite1', src: 'sprite.png', x: 100, y: 200 }]
    };
  ...
  it('should create template with background dimensions', () => {
    ...
    expect(template.name).toBe('New Template');
    expect((template as any).backgroundDimensions).toEqual(backgroundDimensions);
    expect(template.sprites.length).toBe(1);
  });
});
